...
import { loadEnvironmentFile } from "../server.mjs";

const environment = loadEnvironmentFile({ ...process.env });
const required = ["WORKATO_API_KEY", "WORKATO_IDP_USER_ID", "WORKATO_GENIE_HANDLE"];
const missing = required.filter((name) => !environment[name]);
if (environment.GENIE_E2E !== "1" || missing.length) {
  console.error(`Refusing to contact a workspace. Set GENIE_E2E=1 and: ${missing.join(", ") || "none missing"}`);
  process.exit(2);
}

const child = spawn(process.execPath, ["server.mjs"], { cwd: new URL("..", import.meta.url), env: { ...environment, PORT: "0" }, stdio: ["ignore", "pipe", "inherit"] });
let serverUrl;
...
try {
  ...
  const stream = await fetch(`${serverUrl}/api/conversations/${encodeURIComponent(conversation.conversation_id)}/messages`, {
    method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ message: environment.GENIE_E2E_MESSAGE ?? "Reply with exactly: SDK_E2E_OK" })
  });
