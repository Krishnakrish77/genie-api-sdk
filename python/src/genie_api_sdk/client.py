...
class GenieClient:
    ...
    def get_conversation(self, genie_handle: str, conversation_id: str) -> Conversation:
        data = self._data(self._safe_get(self._path(genie_handle, f"/conversations/{self._id(conversation_id)}")))
        return Conversation.from_dict(data)
    ...
    def list_messages(self, genie_handle: str, conversation_id: str, *, limit: Optional[int] = None, cursor: Optional[str] = None) -> Page[Message]:
        data = self._data(self._safe_get(self._path(genie_handle, f"/conversations/{self._id(conversation_id)}/messages"), params={"limit": limit, "cursor": cursor}))
    ...
    def send_message(self, genie_handle: str, conversation_id: str, message: str, *, file_id: Optional[str] = None) -> Run:
        data = self._data(raise_for_status(self._client.post(self._path(genie_handle, f"/conversations/{self._id(conversation_id)}/messages"), json=self._json(message=message, file_id=file_id, stream=False), headers=self._headers())))
    ...
    def list_events(self, genie_handle: str, *, since_created_at: Optional[str] = None, conversation_id: Optional[str] = None, limit: Optional[int] = None) -> Page[Event]:
        data = self._data(self._safe_get(self._path(genie_handle, "/conversations/events"), params={"since_created_at": since_created_at, "conversation_id": conversation_id, "limit": limit}))
